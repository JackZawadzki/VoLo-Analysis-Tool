...
-- Standalone per-company document store. This is the substrate the two-pass
-- LLM reads to produce each company's update. Populated by the portfolio
-- ingestion (Drive folder walk + Granola), which replicates VoLo Mind's
-- transfer pattern but keeps Portfolio Review self-contained (no dependency
-- on VoLo Mind being enabled). One row per (company, source, source_doc_id);
-- content_hash makes re-ingestion idempotent.
CREATE TABLE IF NOT EXISTS pr_documents (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    company_id      INTEGER NOT NULL REFERENCES pr_companies(id) ON DELETE CASCADE,
    source          TEXT    NOT NULL DEFAULT 'drive',     -- drive | granola
    source_doc_id   TEXT    NOT NULL,                      -- Drive fileId or Granola note id
    title           TEXT    NOT NULL DEFAULT '',
    doc_type        TEXT    NOT NULL DEFAULT '',           -- deck | model | board | investor | note | other
    ...
    occurred_at     TEXT,                                  -- doc/meeting date (ISO)
    source_modified TEXT,                                  -- Drive modifiedTime
    fetched_at      TEXT    NOT NULL DEFAULT (datetime('now')),
    UNIQUE(company_id, source, source_doc_id)
);
CREATE INDEX IF NOT EXISTS idx_pr_documents_company ON pr_documents(company_id, occurred_at DESC);

-- One AI-generated company update per run. Wraps the two-pass extraction's
-- audit trail (which docs were used, recon manifest, model) so the UI can show
-- "why". The derisking scores + traction metrics themselves live in their own
-- tables (pr_derisking_scores / pr_traction_snapshots, evaluator='llm'); this
-- row ties a single generation together and timestamps it.
CREATE TABLE IF NOT EXISTS pr_company_updates (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    company_id      INTEGER NOT NULL REFERENCES pr_companies(id) ON DELETE CASCADE,
    period          TEXT    NOT NULL DEFAULT '',           -- e.g. '2026 LLM'
    status          TEXT    NOT NULL DEFAULT 'success',    -- success | partial | failed
    headline        TEXT    NOT NULL DEFAULT '',           -- 1-line "what changed"
    summary         TEXT    NOT NULL DEFAULT '',           -- 2-4 sentence narrative
    model_used      TEXT    NOT NULL DEFAULT '',
    n_docs_seen     INTEGER NOT NULL DEFAULT 0,
    n_docs_used     INTEGER NOT NULL DEFAULT 0,
    manifest_json   TEXT    NOT NULL DEFAULT '',           -- recon pass: doc->field map + gaps
    evidence_json   TEXT    NOT NULL DEFAULT '',           -- per-field cited doc ids
    error_summary   TEXT    NOT NULL DEFAULT '',
    created_at      TEXT    NOT NULL DEFAULT (datetime('now'))
);
CREATE INDEX IF NOT EXISTS idx_pr_company_updates_company ON pr_company_updates(company_id, created_at DESC);
"""
