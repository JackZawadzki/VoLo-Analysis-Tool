...
# ── Reworked Portfolio view ───────────────────────────────────────────────────
# The portfolio roster comes from the Drive folder (one subfolder = one company,
# ingested into pr_documents). Each company's update is produced by the two-pass
# LLM (portfolio_update.py) and stored as a traction snapshot + derisking score.
# These endpoints power the native-SPA Portfolio tab.
def _months_until(date_str, today_str):
    try:
        from datetime import datetime
        d = datetime.fromisoformat(str(date_str)[:10]).date()
        t = datetime.fromisoformat(str(today_str)[:10]).date()
        return (d.year - t.year) * 12 + (d.month - t.month)
    except Exception:
        return None


def _pr_company_flags(company, traction, derisk, ret, today):
    """Exception list driving the dashboard's Attention strip.
    Levels: 'alert' (act now) > 'warn' > 'info'."""
    flags = []
    rm = traction.get("runway_months")
    if rm is not None and rm <= 9:
        flags.append({"kind": "runway", "level": "alert" if rm <= 4 else "warn",
                      "label": f"Runway ~{int(rm)} mo"})
    else:
        fume = company.get("fume_date")
        if fume:
            m = _months_until(fume, today)
            if m is not None and m <= 6:
                flags.append({"kind": "runway", "level": "alert" if m <= 3 else "warn",
                              "label": f"Cash runway ~{max(m, 0)} mo (FUME {str(fume)[:10]})"})
    g = traction.get("revenue_growth_pct")
    if g is not None and g <= -0.10:
        flags.append({"kind": "revenue", "level": "warn",
                      "label": f"Revenue {abs(g) * 100:.0f}% YoY decline"})
    mult = (ret or {}).get("multiple")
    if mult is not None and mult < 1.0:
        flags.append({"kind": "markdown", "level": "warn",
                      "label": f"Marked below cost ({mult:.2f}x)"})
    q = (derisk or {}).get("quartile")
    if q is not None and q <= 1 and not (derisk or {}).get("is_exited"):
        flags.append({"kind": "derisk", "level": "warn", "label": "Bottom-quartile derisking"})
    fr = (traction.get("fundraising_status") or company.get("next_round_expect") or "").strip()
    if fr and fr.lower() not in ("n/a", "none", "no", "-"):
        flags.append({"kind": "fundraise", "level": "info", "label": f"Raising: {fr}"})
    return flags


@api.get("/overview")
def api_pr_overview(user: CurrentUser = Depends(get_current_user)):
    """Dashboard bundle: totals, an attention/exception list, and a holdings
    table — each holding carrying its latest AI update (traction + derisking)."""
    ...
    try:
        ...
        for c in companies:
            cd = _row_to_dict(c)
            cid = cd["id"]
            tr = conn.execute("SELECT * FROM pr_traction_snapshots WHERE company_id=? "
                              "ORDER BY id DESC LIMIT 1", (cid,)).fetchone()
            tr = _row_to_dict(tr) if tr else {}
            der = conn.execute("SELECT * FROM pr_derisking_scores WHERE company_id=? "
                               "ORDER BY id DESC LIMIT 1", (cid,)).fetchone()
            der = _row_to_dict(der) if der else {}
            upd = conn.execute("SELECT * FROM pr_company_updates WHERE company_id=? "
                               "ORDER BY id DESC LIMIT 1", (cid,)).fetchone()
            upd = _row_to_dict(upd) if upd else {}
            ndocs = conn.execute("SELECT COUNT(*) AS n FROM pr_documents WHERE company_id=?",
                                 (cid,)).fetchone()["n"]
            ret = conn.execute("SELECT * FROM pr_returns WHERE company_id=? "
                               "ORDER BY as_of_date DESC LIMIT 1", (cid,)).fetchone()
            ...
            totals["n_companies"] += 1
            totals["n_reviewed"] += 1 if upd else 0
            totals["n_docs"] += ndocs
            totals["n_flags"] += len(flags)

            holdings.append({
                "id": cid, "name": cd["name"], "fund": cd.get("fund"), "sector": cd.get("sector"),
                "commercial_status": tr.get("commercial_status") or cd.get("commercial_status"),
                "revenue_current": tr.get("revenue_current"),
                "revenue_growth_pct": tr.get("revenue_growth_pct"),
                "runway_months": tr.get("runway_months"),
...
@api.get("/company/{company_id}/full")
def api_pr_company_full(company_id: int, user: CurrentUser = Depends(get_current_user)):
    """Everything the company-detail view needs: the company, its latest AI
    update (traction + derisking with per-dimension reasoning + evidence), the
    document corpus that update was built from, derisking history, and notes."""
    from .derisking import DIMENSIONS
    conn = get_db()
    try:
        c = conn.execute("SELECT * FROM pr_companies WHERE id=?", (company_id,)).fetchone()
        if not c:
            raise HTTPException(404, f"Company {company_id} not found")
        company = _row_to_dict(c)

        upd_row = conn.execute("SELECT * FROM pr_company_updates WHERE company_id=? "
                               "ORDER BY id DESC LIMIT 1", (company_id,)).fetchone()
        update = _row_to_dict(upd_row) if upd_row else None
        evidence_gaps = []
        if update:
            try:
                evidence_gaps = (json.loads(update.get("manifest_json") or "{}") or {}).get("evidence_gaps", [])
            except Exception:
                evidence_gaps = []

        tr_row = conn.execute("SELECT * FROM pr_traction_snapshots WHERE company_id=? "
                              "ORDER BY id DESC LIMIT 1", (company_id,)).fetchone()
        traction = _row_to_dict(tr_row) if tr_row else None

        derisk = [_row_to_dict(r) for r in conn.execute(
            "SELECT * FROM pr_derisking_scores WHERE company_id=? ORDER BY id", (company_id,)
        ).fetchall()]
        # Reasoning + evidence come from the most recent LLM-scored row.
        lr = conn.execute(
            "SELECT reasoning_json FROM pr_derisking_scores WHERE company_id=? AND evaluator='llm' "
            "ORDER BY id DESC LIMIT 1", (company_id,)).fetchone()
        latest_reasoning = {}
        if lr:
            try:
                latest_reasoning = json.loads(lr["reasoning_json"] or "{}") or {}
            except Exception:
                latest_reasoning = {}

        documents = [_row_to_dict(r) for r in conn.execute(
            "SELECT id, source, title, doc_type, occurred_at, source_url, body_tokens "
            "FROM pr_documents WHERE company_id=? "
            "ORDER BY COALESCE(occurred_at,'') DESC, id DESC LIMIT 200", (company_id,)
        ).fetchall()]

        returns = [_row_to_dict(r) for r in conn.execute(
            "SELECT * FROM pr_returns WHERE company_id=? ORDER BY as_of_date", (company_id,)
        ).fetchall()]
        board = [_row_to_dict(r) for r in conn.execute(
            "SELECT * FROM pr_board_seats WHERE company_id=? AND active=1", (company_id,)
        ).fetchall()]
        investments = [_row_to_dict(r) for r in conn.execute(
            "SELECT * FROM pr_investments WHERE company_id=? ORDER BY investment_date", (company_id,)
        ).fetchall()]
        try:
            notes = [_row_to_dict(r) for r in conn.execute(
                "SELECT id, note_title, note_url, note_summary, note_updated_at "
                "FROM pr_granola_notes WHERE company_id=? "
                "ORDER BY COALESCE(note_updated_at, fetched_at) DESC LIMIT 10", (company_id,)
            ).fetchall()]
        except Exception:
            notes = []
...
def api_pr_generate_update(company_id: int,
    ...
    fresh traction snapshot + derisking score. Sync; ~20-60s for one company."""
    if user.role != "admin":
        raise HTTPException(403, "Only admins can generate updates")
    from .portfolio_update import generate_update, DEFAULT_MODEL
    conn = get_db()
    try:
        return generate_update(conn, company_id, period=period, model=model or DEFAULT_MODEL)
    except ValueError as e:
        raise HTTPException(400, str(e))
    except HTTPException:
        raise
    except Exception as e:
        logger.exception("Update generation failed")
        raise HTTPException(500, f"Update generation failed: {e}")
    finally:
        conn.close()


@api.post("/drive/discover-companies")
def api_pr_discover_companies(parent_folder_id: Optional[str] = Query(None),
                              user: CurrentUser = Depends(get_current_user)):
    """Source the portfolio roster from the Drive folder: one subfolder = one
    company. Upserts pr_companies + links the folder. Admin only."""
    if user.role != "admin":
        raise HTTPException(403, "Only admins can run discovery")
    from ..routes.drive import _get_drive_service
    ...
    try:
        service = _get_drive_service(user.id)
        return discover_companies(conn, service, folder)
    except HTTPException:
        raise
    except Exception as e:
...
@api.post("/company/{company_id}/ingest")
def api_pr_ingest_company(company_id: int, user: CurrentUser = Depends(get_current_user)):
    """Walk a company's Drive folder(s), extract every supported file into
    pr_documents, and mirror Granola notes. Admin only."""
    if user.role != "admin":
        raise HTTPException(403, "Only admins can ingest")
    from ..routes.drive import _get_drive_service
    from .ingest import ingest_company
    conn = get_db()
    try:
        service = _get_drive_service(user.id)
        return ingest_company(conn, service, company_id)
    except HTTPException:
        raise
    except Exception as e:
...
def api_pr_generate_all(period: str = Query("2026 LLM"),
    ...
    and LLM-bound, so this is slow (minutes for a large portfolio)."""
    if user.role != "admin":
        raise HTTPException(403, "Only admins can generate updates")
    from .portfolio_update import generate_update, DEFAULT_MODEL
    conn = get_db()
    try:
        ...
        rows = conn.execute(
            "SELECT DISTINCT company_id FROM pr_documents").fetchall()
        for r in rows:
            cid = r["company_id"]
            try:
                res = generate_update(conn, cid, period=period, model=model or DEFAULT_MODEL)
                done.append({"company_id": cid, "headline": res.get("headline")})
            except Exception as e:
                errors.append({"company_id": cid, "error": str(e)})
        return {"generated": len(done), "results": done, "errors": errors}
    finally:
        conn.close()
