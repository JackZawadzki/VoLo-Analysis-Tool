...
Auth rules:
  - Only @voloearth.com emails can register
  - Each email can only register ONE account
  - Register with email + username + password → immediately active
  - Login with email + password → JWT token (72-hour expiry)
...
def register(req: RegisterRequest):
    """
    Register a new account. Rules:
      - Must be a @voloearth.com email
      - Each email can only register once
      - Account is immediately active (no verification step)
    """
...
def login(req: LoginRequest):
    """
    Log in with email + password. Returns a JWT token valid for 72 hours.
    """
    email = req.email.strip().lower()
    password = req.password
