...
(function () {
    ...
    el.innerHTML = `
      <div class="pf-head">
        <div>
          ...
          <p class="pf-sub">Roster synced from the portfolio Drive folder. Each company's update is generated by an LLM that reads its documents file-by-file.</p>
        </div>
      </div>
    ...
    return `<div class="pf-card pf-admin">
      <div class="pf-admin-row">
        <div class="pf-admin-info">
          ...
          <span class="pf-dim"> — roster + documents from the portfolio Drive folder.</span>
  ...
  async function adminReset() {
    ...
    try {
      ...
      msg.innerHTML = `<div class="pf-ok pf-tiny">Deleted ${r.deleted_companies || 0} companies. Now click <strong>Sync from Drive</strong> to repopulate cleanly.</div>`;
  ...
  async function adminSync() {
    ...
    msg.innerHTML = `<div class="pf-gen-busy"><span class="pf-spin"></span> Building the company roster from the Drive folder…</div>`;
    try {
      const r = await postJSON(`${API}/sync-all`);
      const d = r.discover || {};
      const skip = d.excluded ? `, hid ${d.excluded} non-company folder${d.excluded === 1 ? '' : 's'}` : '';
      msg.innerHTML = `<div class="pf-ok pf-tiny">Roster: ${d.companies_created || 0} new / ${d.companies_updated || 0} existing companies${skip}. Open a company → <strong>Sync &amp; update</strong>, or <strong>Process all</strong> to pull every company's documents.</div>`;
  ...
  function notesCard(notes) {
    if (!notes.length) {
      return '';  // Granola deferred — hide the notes section until notes exist.
    }
