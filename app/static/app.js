...
function wizRenderReport(r) {
    ...
    html += `<div class="rpt-cover">
        ...
        `)}
        ${(ov.is_blended_followon && r.total_position_forecast) ? `<p style="font-size:0.82rem;color:var(--text-secondary);margin:6px 0 8px;line-height:1.5;">Two return views — <strong>This Check</strong> (return on the new capital, the decision) and <strong>Total Position</strong> (return on all capital deployed, which includes markup already accrued on the priors).</p>
        <div style="font-size:0.72rem;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;color:var(--accent,#5B7744);margin:0 0 5px;">This Check &mdash; new $${fmt(ov.check_size_millions,1)}M at ${fmt(ov.new_check_ownership_pct,1)}%</div>` : (ov.is_blended_followon ? `<p style="font-size:0.8rem;color:#92400e;margin:6px 0 8px;line-height:1.5;background:#fef6e7;border:1px solid #f0c674;border-radius:5px;padding:8px 12px;">&#9888; Follow-on detected, but this saved report predates the <strong>This Check / Total Position</strong> split, so the figure below is the older blended (total-position) number — <strong>re-run the deal</strong> to get the new breakdown.</p>` : '')}
    ...
    if (_hasSens || _hasVar) {
        ...
        if (_hasSens) {
            ...
            <p class="rpt-narrative" style="font-size:12px;color:var(--text-secondary,#6b7280);margin:0 0 6px;">Each input moved &plusmn;20&ndash;30% on its own; the bar shows the resulting MOIC range &mdash; <span style="color:#dc2626;font-weight:600;">red = downside</span>, <span style="color:#16a34a;font-weight:600;">green = upside</span> &mdash; biggest first. Symmetric bars scale MOIC evenly; an off&#8209;centre bar (e.g. pre&#8209;money) is a genuinely convex input.</p>`;
            // Two-sided tornado: red (downside) / green (upside) diverging from the base.
            ...
            trows.forEach(t => {
                ...
                html += `<div style="display:flex;align-items:center;gap:10px;margin:3px 0;">
                    <div style="width:140px;font-size:11px;text-align:right;color:#374151;flex-shrink:0;">${t.input}</div>
...
const DD_FIELDS = [
    ...
    { key: 'time_to_launch_years', label: 'Time to Commercial Launch', unit: 'whole yrs', step: 1, dft: [2, 1, 0] },
    { key: 'projection_years', label: 'Hold / Horizon', unit: 'yrs total · pre-launch eats into it', step: 1, dft: [10, 10, 10], driver: false },
    { group: 'Margins & Costs' },
    ...
    { key: 'opex_commercial_m', label: 'Operating Spend — commercial', unit: '$M/yr · +15%/yr', step: 0.5, dft: [6, 6, 6] },
    { key: 'opex_prelaunch_m', label: 'Operating Spend — pre-launch', unit: '$M/yr', step: 0.5, dft: [3, 3, 3] },
...
function _ddRenderCharts(pnl, cs) {
    ...
    const grid = document.querySelector('.dd-charts-grid');
    if (_dd.chartMode === 'all') { if (grid) grid.classList.add('dd-charts-stack'); _ddRenderChartsAll(); return; }
    if (grid) grid.classList.remove('dd-charts-stack');
